 ...
 * ClassName：用户信息控制器
 */
Ext.define("CM.controller.MgtReviewCtrl", {
	extend:'Ext.app.Controller',
	init:function(app) {
		var addWin;
		Ext.QuickTips.init();
		this.control({
			'mgtreviewlist actioncolumn[itemId=file]': {
		     	click:this.download
		    },
			'mgtreviewlist button[itemId=add]': {
				click:function() {
					if(!addWin) {
						addWin = Ext.create("CM.view.form.MgtReviewAddWin");
					}
					addWin.show();	
				}
			},
			'mgtreviewaddwin button[itemId=submit]':{
				click:this.submitForm
			},
			'mgtreviewaddwin button[itemId=cancel]':{
				click:this.closeWin
			}
     	});
     	this.addTab();
		...
		if(sForm.itemId.toLowerCase().indexOf('add') > -1) {
			...
			sForm.submit({
				...
				success:function(form, action){
					var recGrid = Ext.getCmp('mgtReviewList');
	...
	addTab:function() {
		...
		mainTab.add({ 
			itemId: 'mgtReviewCtrl', //向mainTab的items项中,添加的itemId
    		title: '管理评审',
    		layout:'border',
    		items:[{
    			region:'center',
